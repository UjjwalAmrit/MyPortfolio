...
app.post("/contact", (req, res) => {
    ...
    db.query(checkEmailQuery, [email], (err, result) => {
        ...
        if (result.length > 0) {
            // If the email exists, update the existing record (feedback and contact form data)
            const updateQuery = `
                UPDATE feedback 
            ...
            db.query(updateQuery, [message, email], (err) => {
                if (err) {
                    ...
                    return res.redirect("/");  // Redirect back to the home page after submission
                }
            });
        } else {
            // If the email doesn't exist, insert a new record for the contact form
